...
function t.CreateMain(H)
    ...
    function u.ReloadMain(v)
        ...
        if
            v ~= "" and type(v) == "string" and string.find(v:lower(), ".webm") and
                pcall(
                    function()
                        writefile("seahub.webm", kramp.request({Url = v}).Body)
                    end
                )
         then
            wait(.25)
    ...
    if
        au ~= "" and type(au) == "string" and string.find(au:lower(), ".webm") and
            pcall(
                function()
                    writefile("seahub.webm", kramp.request({Url = au}).Body)
                end
            )
     then
        wait(.25)
...
function SaveSettings()
    ...
    writefile("Perfect Hub/" .. SaveFileName, HttpService:JSONEncode(Settings))
end
...
function ReadSetting() 
    local s,e = pcall(function() 
        ...
        return HttpService:JSONDecode(readfile("Perfect Hub/" .. SaveFileName))
    end)
    if s then return e 
    else
        SaveSettings()
...
if table.find(keylist,key)  then
    ...
    function ditme() 
        ...
        local Page1 = Main.CreatePage({Page_Name = 'Rolls', Page_Title = 'Roll Tab'})
            ...
            spawn(function() 
                if Settings.AutoRollMutation then 
                    local request = function(...) 
                        if Settings.MutationWebhook then return kramp.request(...) end
                    end
                    local plrHive
                    repeat wait()
...
function Anti1() 
    ...
    for i, v in pairs(GetListField()) do
        if v ~= "Perfect Hub Winning Field" then
...
GUISettingSection.CreateBind({Title = 'Toggle GUI', Key = Enum.KeyCode.RightControl}, function()
    ...
    if game.CoreGui:FindFirstChild("Perfect Hub GUI")then for a,b in ipairs(game.CoreGui:GetChildren())do if b.Name=="Perfect Hub GUI"then b.Enabled=getgenv().GuiToggled end end end
end)
if getgenv().Key == nil then
    GUISettingSection.CreateButton({Title = 'Destroy GUI'}, function()
	    if game.CoreGui:FindFirstChild("Perfect Hub GUI")then for a,b in ipairs(game.CoreGui:GetChildren())do if b.Name=="Perfect Hub GUI"then b:Destroy()end end end
    end)
end
-- UI LIB --
            ...
            if Settings.Farm and HumanoidRootPart then
                ...
                if token then
                    if getgenv().IsValidTokenPos(token, TempField,FieldPosIn) and getgenv().IsToken(token) and not getgenv().IsTokenBlacklist(token) and Settings.Farm and ValidFarm() then
                     ...
                     if TempField ~= "Perfect Hub Winning Field" and plr.Character:FindFirstChild("HumanoidRootPart") then 
